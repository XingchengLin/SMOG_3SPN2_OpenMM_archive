    ...
    if (runId == 0):
        ...
        for line in open(lammps_template).readlines():
            ...
            if len(items) >= 2 and items[0] == "read_data":
                fh.write("read_dump           ../../perturbed.dcd 0 x y z box no format molfile dcd /home/xclin/lib/vmd.old/plugins/LINUXAMD64/molfile\n")
        fh.close()
        ...
        for line in open(lammps_template).readlines():
            ...
            if len(items) >= 2 and items[0] == "read_data" and runId >= 1:
                fh.write(line)
                cmd = '/home/xclin/bin/anaconda2/bin/mdconvert --force -i %d -o DUMP_Extract.dcd ../run%03d/DUMP_FILE.dcd' % (-1, runId - 1) 
