...
# Shuming Liu modified this script to adapt to his own system
...
def removeIDR_for_dihedrals_section(inputFile, outputFile):
    ...
    for i in my_lt_range(0, np.shape(dihedrals_interactions)[0], 1):
        dihedral_i = int(dihedrals_interactions[i][0]) % num_CA
        dihedral_j = int(dihedrals_interactions[i][1]) % num_CA
        dihedral_k = int(dihedrals_interactions[i][2]) % num_CA
        dihedral_l = int(dihedrals_interactions[i][3]) % num_CA
        if (dihedral_i not in histonetailSegment) and (dihedral_j not in histonetailSegment) and (dihedral_k not in histonetailSegment) and (dihedral_l not in histonetailSegment):
