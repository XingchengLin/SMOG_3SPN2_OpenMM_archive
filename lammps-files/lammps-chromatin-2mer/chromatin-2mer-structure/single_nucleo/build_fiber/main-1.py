...
nrl = int(sys.argv[1])  # The nucleosome repeat length, change this manually
seq_fake_DNA = sys.argv[2]  # sequence of fake DNA, with length (nrl - 165)
num_copy = int(sys.argv[3])  # The number of nucleosomes in the fiber
rot_x, rot_y, rot_z = float(sys.argv[4]), float(sys.argv[5]), float(
    sys.argv[6])  # rotation angle of fake DNA relative to 1zbb_part-168.pdb
